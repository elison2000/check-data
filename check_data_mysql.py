...
"""
####################################################################################################
#  Name        :  check_data_mysql.py
#  Author      :  Elison
#  Email       :  Ly99@qq.com
#  Description :  检查mysql主备数据是否一致
#  Updates     :
#      Version     When            What
#      --------    -----------     -----------------------------------------------------------------
#      v1.0        2017-08-18
#      v2.0        2022-02-25      使用diff替换sqlite3
#      v2.1.0      2022-04-11      增加核对模式，支持mysql,doris,tidb
#      v2.1.1      2022-06-12      修复导出sql bug
####################################################################################################
"""
...
def get_args():
    ...
    parser.add_argument("-m", "--mode", type=str, default='fast', choices=['fast', 'slow'],
...
class Mysql:
    ...
    def query_bigtb(self, sql, size=1000):
        cur = self.conn.cursor(cursorclass=MySQLdb.cursors.SSCursor)
        cur.execute(sql)
        while True:
...
def get_tables():
    "获取表名"
    global CONFIG
    source_conf = {'host': CONFIG.source_host, 'port': CONFIG.source_port, 'db': CONFIG.source_db,
                   'user': CONFIG.user, 'password': CONFIG.password, 'charset': CONFIG.charset}
    target_conf = {'host': CONFIG.target_host, 'port': CONFIG.target_port, 'db': CONFIG.target_db,
                   'user': CONFIG.user, 'password': CONFIG.password, 'charset': CONFIG.charset}
    res1 = mysql_query(source_conf, 'show tables')
    res2 = mysql_query(target_conf, 'show tables')
    source_tables = [i[0] for i in res1]
    target_tables = [i[0] for i in res2]
...
class Table:
    def __init__(self, table_name):
        ...
        self.source_conf = {'host': CONFIG.source_host, 'port': CONFIG.source_port, 'db': CONFIG.source_db,
                            'user': CONFIG.user, 'password': CONFIG.password, 'charset': CONFIG.charset}
        self.target_conf = {'host': CONFIG.target_host, 'port': CONFIG.target_port, 'db': CONFIG.target_db,
                            'user': CONFIG.user, 'password': CONFIG.password, 'charset': CONFIG.charset}
    ...
    def get_columns(self):
        "获取列信息"
        sql = "desc `{0}`".format(self.name)
        self.all_columns = mysql_query(self.source_conf, sql)
    ...
    def get_primary_key(self, keys=None):
        ...
        if keys:
            ...
            self.primary_key = [i[0] for i in self.all_columns if i[3] == 'PRI']
        if self.primary_key:
            self.primary_key_quote = ['`{0}`'.format(i) for i in self.primary_key]
...
def get_target_data(obj):
    ...
    target_conn = Mysql(obj.target_conf)
    ...
    if chg_id_list:
        ...
        for pk_id_text in chg_id_list:
            ...
            export_cmd = """mysqldump -h{} -P{} -u{} -p'{}' {} {} --single-transaction --no-create-info --compact --replace --default-character-set={} --where="{}" | grep "REPLACE INTO" >>{}""".format(
                ...
                obj.source_conf['db'], obj.name, obj.source_conf['charset'], where_clause, filename)
